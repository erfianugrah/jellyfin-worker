...
async function handleRequest(request) {
  ...
  const rangeCacheKey = `${newRequest.hostname}${newRequest.pathname}${
    subRequest.headers.get("Range")
  }`;
  const queryCacheKey =
    `${newRequest.hostname}${newRequest.pathname}${newRequest.search}`;

  const cacheAssets = [
    {
      asset: "transcode",
      key: customCacheKey,
      regex: /(.*\.(m4s|ts|webm|m2ts|mts))/,
      info: 0,
      ok: 31556952,
      redirects: 30,
      clientError: 10,
    ...
    },
    {
      asset: "direct",
      key: rangeCacheKey,
      regex: /(.*\.(avi|mpeg|mpg|mkv|bin|webm|vob|flv|3gp|m4v|wmv|qt))/,
      info: 0,
      ok: 31556952,
      redirects: 30,
      clientError: 10,
    ...
    },
    {
      ...
      key: queryCacheKey,
      regex:
        /(.*\.(jpg|jpeg|png|bmp|pict|tif|tiff|webp|gif|heif|exif|bat|bpg|ppm|pgn|pbm|pnm))/,
      info: 0,
      ok: 3600,
      redirects: 30,
      clientError: 10,
