...
const media = new Map ([
    ...
    if (newRequest.match(media.get('audio')))
        return await fetch(newRequest, { cf: {cacheEverything: true, cacheTtlByStatus: { '200-299': 86400, '400-499': 5, '500-599': 1 }},
        })
    if (newRequest.match(media.get('directPlay')))
        return await fetch(newRequest, { cf: {cacheEverything: true, cacheTtlByStatus: { '200-299': 86400, '400-499': 5, '500-599': 1 }},
        })
    if (newRequest.match(media.get('manifest')))
